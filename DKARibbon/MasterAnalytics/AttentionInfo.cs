...
namespace DKAExcelStuff
{
    class AttentionInfo
    {
        ...
        private readonly AttentionInfo[] attInfoA;  

        public AttentionInfo() {}

        public AttentionInfo(string attInfo)
        {
            try
            {
                attInfo.Trim();
                ...
                if (IsMultiLinePO)
                {
                    ...
                    attInfoA = new AttentionInfo[QPOSources];
                    for (int i = 0; i < QPOSources; i++)
                    {
                        AttentionInfo ai = new AttentionInfo();
                        attInfoA[i] = ai;
                    }
                }                
                ...
                if (IsMultiLinePO)
                {
                    ...
                    {
                        attInfoA[i].POSourceType = POSourceType;
                        attInfoA[i].CreatedBy = CreatedBy;
                        attInfoA[i].POSourceCode = multiLinePOSourceCodesL[i];
                        attInfoA[i].Requester = GetRequester(reqL, i);                           
                    }
                }
                else
                {
            ...
            {
                POSourceType = "Can't Read";
                POSourceCode = "Can't Read";
                Requester = "Can't Read";
                CreatedBy = "Can't Read";
                AttInfo = "Can't Read";
            }            
        }
        ...
        // indexer stuff
        public AttentionInfo this[int i]
        {
            get => attInfoA[i];
            set => attInfoA[i] = value;
        }
        public int GetQPO => attInfoA.Count();
